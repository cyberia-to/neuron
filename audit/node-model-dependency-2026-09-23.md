...
> Recorded from cyberia-to/neuron#7 (branch `fix/node-model-dependency`), closed 2026-09-24 as superseded by main; evidence preserved as written. Since then main renamed the `cell-*` packages to `neuron-*` (3be383a), so the Cargo-alias boundary described below no longer exists; the `neuron-id` / `neuron-model` contracts it validates are on main.
